...
class VectorStore:
    def __init__(self):
        ...
        logger.debug(f"QdrantClient initialized with host={app_settings.VECTOR_DB_HOST}, port={app_settings.VECTOR_DB_PORT}")
    ...
    def add_document_chunks(
        ...
        try:
            logger.info(f"Adding {len(chunk_texts)} chunks for document_id={document_id} to vector store.")
    ...
    def search_chunks(
        ...
        try:
            logger.info(f"Searching for top {top_k} chunks for document_id={document_id}.")
            query_filter = None
            if document_id:
                query_filter = Filter(
                    must=[
            ...
            logger.info(f"Search completed for document_id={document_id}. Found {len(results)} results.")
            return response
        except Exception as e:
    ...
    def delete_document_chunks(self, document_id: str) -> bool:
        try:
            logger.info(f"Deleting chunks for document_id={document_id} from vector store.")
            self.client.delete(
                collection_name=COLLECTION_NAME,
