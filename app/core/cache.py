...
class RedisCache:
    ...
    def cache_query(self, document_id: str, question: str, answer: str, ttl: Optional[int] = None) -> bool:
        """Cache a query result"""
        key = self._generate_query_key(document_id, question)
        return self.set("query_result", key, {"answer": answer}, ttl)
    
    def get_cached_query(self, document_id: str, question: str) -> Optional[Dict[str, str]]:
        """Get a cached query result"""
        key = self._generate_query_key(document_id, question)
        return self.get("query_result", key)
    
    def cache_document_chunks(self, document_id: str, chunks: Any, ttl: Optional[int] = None) -> bool:
