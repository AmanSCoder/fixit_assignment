...
class RAGEngine:
    ...
    async def process_query(self, document_id: str, question: str) -> Tuple[str, float, List[str]]:
        ...
        """
        logger.info(f"Processing query for document_id={document_id}, question='{question}'")
        ...
        logger.debug(f"Searching for relevant chunks in vector store for document_id={document_id}")
        ...
        if not search_results["documents"] or not search_results["documents"][0]:
            logger.warning(f"No relevant information found for document_id={document_id}, question='{question}'")
        ...
        # Cache the result
        logger.debug(f"Caching result for document_id={document_id}, question='{question}'")
    ...
    async def process_query_stream(self, document_id: str, question: str) -> AsyncGenerator[Tuple[str, Optional[List[str]]], None]:
        ...
        """
        logger.info(f"Processing streaming query for document_id={document_id}, question='{question}'")
        # Check cache first
        cached_result = cache.get_cached_query(document_id, question)
        if cached_result:
        ...
        logger.debug(f"Caching streamed result for document_id={document_id}, question='{question}'")
        cache.cache_query(document_id, question, full_answer)
        logger.info(f"Streaming query processed and cached for document_id={document_id}, question='{question}'")
